...
export function Menu() {
    ...
    return (
        <> {shown ? <div className="grid grid-rows-[auto,_1fr] fixed right-0 top-0 bottom-0 h-full w-1/2 md:w-1/6  py-5 md:py-10 px-4 md:px-8 bg-white z-50 motion-preset-slide-left-md motion-preset-fade-lg">
            ...
            <button onClick={() => set(true)} className="fixed top-4 right-4 bg-white shadow-2xl p-5 rounded-full text-primary motion-duration-1000 motion-ease-in-out hover:motion-scale-out-75">
                <RiMenu4Line />
            </button>
        }</>
    )
